...
public:
    ...
    bool Init(SDL_Window* window, nri::Format swapChainFormat, uint32_t queuedFrameNum, nri::Queue* queue)
    {
        ...
        if (deviceDesc.graphicsAPI == nri::GraphicsAPI::D3D12)
        {
            ...
            {
                Logs::Error("NRImGui: falha ao obter objetos D3D12 nativos.");
            ...
            if (FAILED(hr))
            {
                Logs::Error("NRImGui: falha ao criar descriptor heap D3D12: %d", static_cast<int>(hr));
                ImGui_ImplSDL3_Shutdown();
                ImGui::DestroyContext();
                return false;
            }
            ...
            initInfo.SrvDescriptorAllocFn =
                   ...
                   D3D12_GPU_DESCRIPTOR_HANDLE* outGpuHandle)
                {
                    ...
                    const UINT incrementSize =
                        info->Device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
            ...
            {
                Logs::Error("NRImGui: falha ao inicializar backend DX12.");
        ...
        else if (deviceDesc.graphicsAPI == nri::GraphicsAPI::VK)
        {
            ...
            if (!vkDevice || !vkQueue)
            {
                Logs::Error("NRImGui: falha ao obter objetos Vulkan nativos.");
                ImGui_ImplSDL3_Shutdown();
                ImGui::DestroyContext();
                return false;
            }
            ...
            initInfo.PipelineRenderingCreateInfo.pColorAttachmentFormats = &VKColorFormat;
            */

            if (!ImGui_ImplVulkan_Init(&initInfo))
            {
                Logs::Error("NRImGui: falha ao inicializar backend Vulkan.");
        ...
        {
            Logs::Error("NRImGui: API grafica nao suportada.");
            ImGui_ImplSDL3_Shutdown();
            ImGui::DestroyContext();
            return false;
        }
    ...
    void EndAndRender(nri::CommandBuffer* nriCommandBuffer, nri::Descriptor* colorAttachment, uint32_t width, uint32_t height)
    {
        ...
        {
            VkCommandBuffer nativeCmdList =
                static_cast<VkCommandBuffer>(ICore.GetCommandBufferNativeObject(nriCommandBuffer));
